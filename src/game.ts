...
// Triggered platform
let platformTriggerBox = new TriggerBoxShape(new Vector3(2, 2, 2), new Vector3(0, 1.7, 0)) // Modified to match platform size
...
// Pathed platform
let path = [new Vector3(6.5, 7, 4), new Vector3(6.5, 7, 12), new Vector3(6.5, 10.5, 12), new Vector3(6.5, 10.5, 4)]
const pathedPlatform = new PathedPlatform(new GLTFShape("models/movingPlatform.glb"), path, 10)

// Coin
let coinTriggerBox = new TriggerBoxShape(new Vector3(1.5, 3, 1.5), new Vector3(0, 1, 0)) // Trigger shape for coin
const coin = new Coin(new GLTFShape("models/starCoin.glb"), new Transform({ position: new Vector3(9, 12.75, 8) }), coinTriggerBox)
